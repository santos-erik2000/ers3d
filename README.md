...
CRM operacional da ERS 3D Soluções e Fabricações. Este README cobre a **Fundação** (Sprint 0 + Sprint 1 do backlog) — setup do projeto, banco, autenticação e permissões por ação —, o módulo **Clientes** (Sprint 2), o **Kanban CRM** (Sprint 3), **Filamentos & Calculadora** (Sprint 4) e **Orçamento & Ciclo mensal** (Sprint 5). Os demais módulos de negócio (Produção, Qualidade, Entrega, Financeiro) entram nos sprints seguintes, conforme a Etapa 5 do planejamento.
...
- Aprovar uma versão (`status: APPROVED`) é só o registro do orçamento aprovado — não move o card do Kanban nem mexe em estoque/financeiro (isso é Sprint 6/9). A transição **Negociação → Desenvolvimento** do Kanban agora checa de verdade a existência de uma `QuoteVersion` aprovada vinculada à oportunidade (`src/modules/crm/services/opportunities.ts`, `validateTransition`), substituindo o `TODO` do Sprint 3
...
- **Testes automatizados atuais usam Prisma mockado**, não um Postgres real — não havia `psql`/`docker` disponível no ambiente em que a Fundação foi implementada, e essa mesma limitação se manteve nos Sprints 2 a 5. Cobre a lógica de negócio (regra do último ROOT, guarda de permissão, formato da auditoria, validação de CPF/CNPJ, detecção de duplicidade, validação de transição de etapa do Kanban e histórico, fórmula de precificação exaustivamente, saldo de estoque nunca negativo, versionamento de orçamento nunca sobrescrevendo uma versão aprovada, fechamento de ciclo exigindo decisão por card) mas não valida constraints reais do banco (índices, cascatas, comportamento de `String[]`/`Decimal`/enums no Postgres) nem concorrência real. As migrations de clientes, oportunidades, filamentos/jobs e orçamento/ciclo (`prisma/migrations/20260713010000_add_customers`, `..._add_opportunities`, `..._add_filaments_and_jobs`, `20260713040000_add_quotes_and_cycles`) também foram escritas manualmente com `prisma migrate diff --from-schema-datamodel <schema anterior> --to-schema-datamodel prisma/schema.prisma --script`, sem `migrate dev` contra um banco real — a migration do Sprint 5, porém, já foi aplicada com `prisma migrate deploy` contra o Neon de desenvolvimento configurado em `.env` (aditiva: só cria tabelas novas e colunas nuláveis, sem risco para os dados existentes).
