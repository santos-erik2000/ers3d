...
CRM operacional da ERS 3D Soluções e Fabricações. Este README cobre a **Fundação** (Sprint 0 + Sprint 1 do backlog) — setup do projeto, banco, autenticação e permissões por ação —, o módulo **Clientes** (Sprint 2), o **Kanban CRM** (Sprint 3), **Filamentos & Calculadora** (Sprint 4), **Orçamento & Ciclo mensal** (Sprint 5), **Produção** (Sprint 6), **Qualidade** (Sprint 7), **Estoque de peças & Entrega** (Sprint 8) e **Financeiro** (Sprint 9). O Dashboard executivo entra no sprint seguinte, conforme a Etapa 5 do planejamento.
...
src/
  ...
  modules/              camada de domínio — uma pasta por módulo de negócio
    ...
    quality/             checklist de qualidade, reprovação e retrabalho (Sprint 7)
    inventory/           estoque de peças — geração automática, venda/descarte/reserva (Sprint 8)
    deliveries/          registro de entrega, checklist de embalagem (Sprint 8)
    finance/             contas a receber, baixa manual, estorno, fluxo de caixa, lucro realizado/potencial (Sprint 9)
    reports/ catalogs/ files/ notifications/ settings/
...
## O que já funciona (Qualidade — Sprint 7)

- **Checklist de qualidade** (`quality_checks`/`quality_check_items`) — roda contra uma `ProductionOrder` já Concluída, com uma lista fixa de 10 itens (dimensões, integridade estrutural, cor, acabamento, resistência, encaixe, quantidade, personalização, limpeza, embalagem), cada um marcável (passou/falhou), com observação e um campo de evidência fotográfica como URL/texto simples (upload real de arquivo é módulo `files`, fora de escopo). Resultado: Aprovado / Reprovado / Aprovado com ressalva. Painel visível em `/crm/[id]`
- **Reprovação exige motivo obrigatório e gera retrabalho automaticamente** (QUAL-2, caso crítico "Qualidade reprovada gera retrabalho"): na MESMA transação do registro do checklist (`src/modules/quality/services/quality.ts`, `submitQualityCheck`), a oportunidade volta para o estágio Desenvolvimento (mesmo histórico/auditoria que uma movimentação normal produziria) e uma nova `ProductionOrder` (status Aguardando, mesmo job da ordem original — permite consumo adicional de material) é aberta para o novo ciclo de impressão, reaproveitando `completeProduction` (Sprint 6) sem recriar a lógica de reconciliação de estoque. **A reprovação original nunca é apagada nem alterada** — continua consultável no histórico, mesmo depois de um retrabalho seguinte ser aprovado (QUAL-3)
- **Aprovação (ou aprovado com ressalva) só registra o resultado** — não mexe em estágio, estoque nem ordem de produção
- **Pré-condição real da transição Teste de Qualidade → Entrega** (substituindo o `TODO` do Sprint 3/6): exige que o `QualityCheck` mais recente da oportunidade tenha resultado Aprovado ou Aprovado com ressalva (`src/modules/crm/services/opportunities.ts`, `validateTransition`)
- **A reprovação agora só acontece pelo checklist formal** — o quadro Kanban (`/crm`) deixou de oferecer o atalho de arrastar o card de Qualidade de volta para Desenvolvimento com um motivo solto (`window.prompt`): esse caminho não registrava checklist nem abria retrabalho, então foi desativado na UI (o card mostra uma mensagem apontando para o painel Qualidade). Isso também resolve o trade-off de UX documentado nos sprints anteriores ("motivo via `window.prompt`") — a reprovação agora usa um formulário desenhado, com o campo de motivo aparecendo condicionalmente
- Nova permissão `quality.manage` (seedada para ROOT e Administrador) guarda a Server Action do checklist (`src/modules/quality/actions.ts`)
- Toda escrita passa por `requirePermission` e é registrada em `audit_logs`

**Fora de escopo deste sprint, deliberadamente**:
- **Checklist configurável de verdade** — hoje é uma lista fixa de itens (`src/modules/quality/format.ts`, `QUALITY_CHECKLIST_ITEMS`), igual para toda oportunidade. Torná-la configurável por tipo de peça/cliente é um refinamento futuro
- **Upload real de evidência fotográfica** — `evidencePhotoUrl` é só um campo de texto/URL por enquanto; upload de arquivo de verdade é módulo `files`, ainda não implementado
- **Pré-condição real da transição Teste de Qualidade → Entrega** ficou pronta neste sprint, mas **Entrega → Concluído** continua como `TODO` (depende dos módulos `deliveries`/`finance`, sprints futuros)
- Entrega, financeiro e dashboard continuam fora de escopo, como nos sprints anteriores

## O que já funciona (Estoque de peças & Entrega — Sprint 8)

- **Estoque de peças** (`inventory_items`/`inventory_movements`) — quando um `QualityCheck` é registrado como Aprovado ou Aprovado com ressalva (nunca Reprovado — isso já é tratado pelo retrabalho do Sprint 7), a MESMA transação de `submitQualityCheck` (`src/modules/quality/services/quality.ts`) gera o `InventoryItem` correspondente, via `createInventoryItemFromProductionInTx` (`src/modules/inventory/services/inventory.ts`) — a quantidade vem de `Job.quantityProduced` quando a ordem de produção tem job vinculado; sem job (orçamento manual), usa 1 como fallback documentado (TODO: não há campo confiável de quantidade produzida nesse caso). O custo unitário é `Job.directCost / Job.quantityProduced` (custo real de produção, não o preço de venda) quando há job, nulo no fallback manual
- **Decisão de arquitetura (mesmo critério do Sprint 7)**: diferente do par `quotes`/`production` e do par `quality`/`crm` — onde o módulo de destino já dependia de volta do módulo de origem, forçando a escrita direta em vez de uma chamada de função —, aqui **não há ciclo**: `inventory` não importa nada de `quality`/`crm`/`production` (só `audit`, igual `filaments`). Por isso `quality.ts` chama `createInventoryItemFromProductionInTx` normalmente, sem duplicar a lógica de criação — o mesmo padrão de `production.ts` chamar `recordMovementInTx` de `filaments.ts` (Sprint 6)
- **Operações de estoque** (reservar, liberar reserva, vender, descartar, ajustar) — `src/modules/inventory/services/inventory.ts`, `recordInventoryMovementInTx`: mesma disciplina de escrita condicional atômica de `FilamentMovement` (Sprint 4), estendida para as quatro colunas relevantes de uma peça (disponível/reservado/vendido/descartado). **Nunca permite vender ou descartar sem saldo disponível suficiente** (INV-2, caso crítico explícito da Etapa 2 §05, "Venda ou descarte sem estoque") — a checagem é um `updateMany` com filtro de saldo mínimo no próprio `WHERE`, não um "ler depois escrever" separado. Suporta quantidade > 1 (venda/descarte parcial de um lote), não trava em unidade única
- **Entrega** (`deliveries`/`delivery_checklist_items`) — método (retirada/entrega própria/Correios/transportadora/motoboy), endereço, responsável pelo recebimento, rastreio, datas prevista/envio/entrega, comprovante como URL/texto simples (mesma decisão do Sprint 7 para evidência de qualidade — upload real é módulo `files`, fora de escopo), status Pendente/Enviado/Entregue, e um checklist de embalagem fixo (conferência da peça, quantidade, acabamento, proteção, identificação, peso, dimensões, foto da embalagem) modelado como tabela própria (mesmo desenho de `QualityCheckItem`, não um JSON solto — permite consultar/editar item a item). Só é possível registrar entrega quando a oportunidade está na etapa Entrega
- **Conexão real com o Kanban** (`src/modules/crm/services/opportunities.ts`, `validateTransition`): a transição **Entrega → Concluído** (case `"ENTREGA"` do switch por `fromStage` — é a única transição real que sai dessa etapa) agora exige produção concluída + qualidade aprovada (ou aprovada com ressalva) + entrega com status Entregue, todas recalculadas em `moveStage` no momento da transição (não reaproveitadas das checagens das transições anteriores, que só valiam para o passo imediatamente seguinte). Um `case "CONCLUIDO"` foi adicionado ao switch por completude — documentado como nunca alcançável na prática, já que `CONCLUIDO` é terminal neste fluxo (`NEXT_STAGE["CONCLUIDO"] = null`). "Pendências financeiras justificadas" foi conectada de verdade no Sprint 9 (ver seção abaixo)
- Novas permissões `inventory.manage` e `deliveries.manage` (seedadas para ROOT e Administrador) guardam as Server Actions dos dois módulos (`src/modules/inventory/actions.ts`, `src/modules/deliveries/actions.ts`)
- **UI**: painel de estoque de peças e painel de entrega em `/crm/[id]` (mesmo padrão dos painéis de Produção/Qualidade), mais uma visão agregada de leitura em `/estoque-pecas` (todas as oportunidades, atalho de navegação para operar em cada uma — habilitada na sidebar)
- Toda escrita passa por `requirePermission` e é registrada em `audit_logs`

**Fora de escopo deste sprint, deliberadamente**:
- **Quantidade produzida de um orçamento manual** (sem `Job` de origem) — usa 1 como fallback documentado; pedir a quantidade real ao usuário nesse caso é refinamento futuro
- **Reenvio (nova `Delivery`) depois de uma entrega já registrada** — o schema permite mais de uma `Delivery` por oportunidade (a pré-condição sempre olha a mais recente), mas a UI de `/crm/[id]` só cria uma entrega quando não existe nenhuma ainda; um fluxo dedicado de reenvio é refinamento futuro
- Dashboard continua fora de escopo, como nos sprints anteriores

## O que já funciona (Financeiro — Sprint 9)

- **Regra mais importante do módulo**: o valor negociado aprovado NUNCA é tratado como dinheiro em caixa. Aprovar uma `QuoteVersion` gera uma `AccountsReceivable` com status **PREVISTO** (FIN-1) — o caixa só se move quando uma `FinancialTransaction` é de fato registrada (baixa manual). `AccountsReceivable`/`PaymentInstallment` são só o registro do que é esperado; `FinancialTransaction` é a ÚNICA fonte que representa dinheiro entrando/saindo de verdade
- **Nascimento da conta a receber** (`src/modules/quotes/services/quotes.ts`, `approveVersion`) — na MESMA transação em que hoje já reserva filamento/cria `ProductionOrder` (quando a versão tem job), agora também cria a `AccountsReceivable` (status PREVISTO, valor = `QuoteVersion.finalValue`) e UMA `PaymentInstallment` inicial cobrindo o valor total. Isso acontece **sempre** que uma versão é aprovada, com ou sem job — diferente da reserva de estoque, que só existe quando há job
- **Divisão em parcelas** (`splitInstallments`, `src/modules/finance/services/receivables.ts`) — redistribui o valor total em N parcelas (distribuição em centavos, sem deriva de arredondamento), só permitida **antes** de qualquer pagamento já registrado
- **Baixa manual** (`recordPayment`, FIN-2) — cria a `FinancialTransaction` (tipo RECEBIMENTO), atualiza `amountPaid`/status da parcela e recalcula o status agregado da `AccountsReceivable` (todas as parcelas pagas → PAGO; alguma paga e alguma não → PARCIALMENTE_PAGO; nenhuma paga e no prazo → PENDENTE; nenhuma paga e vencida → VENCIDO, calculado inline a cada escrita, nunca por um cron). **Suporta pagamento parcial de verdade** — `amount` pode ser menor que o saldo restante da parcela (`PaymentInstallment.amountPaid`, acumulado, distinto de `amount`), nunca exige quitar de uma vez
- **Estorno** (`reverseTransaction`) — **nunca apaga/altera a transação original**: cria uma NOVA `FinancialTransaction` (tipo ESTORNO, valor negativo/compensatório) vinculada à mesma parcela, referenciando a original via `reversesTransactionId` (`@unique` no schema — uma transação nunca pode ser estornada duas vezes, reforçado no banco e no service). Motivo obrigatório
- **Lucro realizado × lucro potencial em estoque** (FIN-4, `src/modules/finance/services/reports.ts`) — deliberadamente mantidos como funções separadas e nunca confundidos: `getRealizedProfit` aplica a margem do job de origem (`(finalPrice - directCost) / finalPrice`) sobre o valor de CADA transação (nunca sobre o valor total do orçamento — inclui o efeito de estornos, que reduzem o lucro reconhecido na mesma proporção); `getPotentialInventoryProfit` soma `(unitPrice - unitCost) × quantityAvailable` de peças ainda disponíveis em estoque — é só uma estimativa do que PODE virar lucro, nunca saldo disponível. Orçamentos manuais (sem job) contam lucro realizado ZERO — margem desconhecida, nunca inventada
- **Fluxo de caixa simples** (`src/modules/finance/services/cashflow.ts`, `getCashflow`) — retorna `receitaRecebida` (RECEBIMENTO menos ESTORNO dentro do período, dinheiro real) e `receitaPrevista` (saldo AINDA NÃO recebido das contas em aberto — PREVISTO/PENDENTE/PARCIALMENTE_PAGO) como campos separados e inequívocos, nunca um campo genérico "receita"
- **Conexão real com o Kanban** — a transição **Entrega → Concluído** agora também exige que exista uma `AccountsReceivable` para a oportunidade (situação financeira conhecida — nasce sempre na aprovação do orçamento). Se o status não for PAGO, exige uma observação obrigatória em `moveStage` justificando a pendência — **nunca bloqueia a conclusão por inadimplência em si**, só exige que fique documentado
- Nova permissão `finance.manage` (seedada só para ROOT e Administrador) guarda toda escrita (`recordPayment`/`reverseTransaction`/`splitInstallments`); `finance.read` (já existia desde a Fundação, seedada para o perfil Contador) cobre só leitura. **Testado explicitamente**: um usuário só com `finance.read` recebe 403 ao tentar qualquer escrita (FIN-3, caso crítico "Contador tenta editar módulo financeiro" — `src/modules/finance/__tests__/actions.test.ts`)
- **UI**: `/financeiro` — visão geral somente leitura (`finance.read`, acessível ao Contador), sem nenhum botão de ação: contas a receber por status, fluxo de caixa do mês corrente, lucro realizado × potencial claramente separados visualmente. Painel financeiro completo (parcelas, dar baixa, estornar, dividir parcelas) em `/crm/[id]` — exige `crm.manage` para visitar a página (mesma trava das demais abas) e `finance.manage` para cada ação de escrita. "Financeiro" habilitado na sidebar
- Toda escrita passa por `requirePermission` e é registrada em `audit_logs`

**Fora de escopo deste sprint, deliberadamente**:
- **Dashboard executivo** consolidando os números deste módulo com os demais — Sprint 10; `getRealizedProfit`/`getPotentialInventoryProfit`/`getCashflow` já existem prontos para alimentá-lo
- **Contas a pagar e conciliação bancária** — fora do MVP, adiado (Fase 6.2 do briefing original)
- **Cancelamento manual de conta a receber/parcela** (`CANCELADO` existe no enum de status por completude do modelo de dados, mas nenhuma função deste sprint o atribui) e **estorno parcial** (`reverseTransaction` só cobre estorno total de uma transação) — refinamentos futuros, não inventados
- **Seletor de período no `/financeiro`** — a tela usa o mês corrente como padrão fixo; um filtro de período customizado na UI é refinamento futuro (as funções de serviço já aceitam qualquer período)
...
- **Testes automatizados atuais usam Prisma mockado**, não um Postgres real — não havia `psql`/`docker` disponível no ambiente em que a Fundação foi implementada, e essa mesma limitação se manteve nos Sprints 2 a 8. Cobre a lógica de negócio (regra do último ROOT, guarda de permissão, formato da auditoria, validação de CPF/CNPJ, detecção de duplicidade, validação de transição de etapa do Kanban e histórico, fórmula de precificação exaustivamente, saldo de estoque de filamento nunca negativo, versionamento de orçamento nunca sobrescrevendo uma versão aprovada, fechamento de ciclo exigindo decisão por card, reserva de filamento na aprovação falhando por completo sem saldo, reconciliação de gramas reais na conclusão da produção, reprovação de qualidade exigindo motivo e abrindo retrabalho na mesma transação, histórico de qualidade preservando reprovações antigas, geração automática de peça em estoque só em aprovações de qualidade nunca em reprovações, venda/descarte de peça nunca deixando saldo negativo — inclusive quantidade > 1 —, transição Entrega → Concluído exigindo as três pré-condições checáveis hoje) mas não valida constraints reais do banco (índices, cascatas, comportamento de `String[]`/`Decimal`/enums no Postgres) nem concorrência real de duas transações simultâneas (o mecanismo de escrita condicional atômica de `recordMovementInTx`/`recordInventoryMovementInTx` é o que garante essa segurança em produção — testado unitariamente, não sob carga real). As migrations de clientes, oportunidades, filamentos/jobs, orçamento/ciclo, produção, qualidade e estoque de peças/entrega (`prisma/migrations/20260713010000_add_customers`, `..._add_opportunities`, `..._add_filaments_and_jobs`, `20260713040000_add_quotes_and_cycles`, `20260713050000_add_production`, `20260713060000_add_quality`, `20260713070000_add_inventory_and_deliveries`) também foram escritas manualmente com `prisma migrate diff --from-schema-datamodel <schema anterior> --to-schema-datamodel prisma/schema.prisma --script`, sem `migrate dev` contra um banco real — as migrations dos Sprints 5 a 8, porém, já foram aplicadas com `prisma migrate deploy` contra o Neon de desenvolvimento configurado em `.env` (todas aditivas: só criam tabelas/enums novos e colunas nuláveis, sem risco para os dados existentes; a do Sprint 6 adiciona dois valores a um enum existente — `RESERVA`/`LIBERACAO_RESERVA` em `FilamentMovementType` — o que é seguro em uma migration própria porque nenhum dado é escrito usando esses valores na mesma transação; a do Sprint 7 só cria as duas tabelas novas de qualidade e o enum `QualityCheckResult`; a do Sprint 8 só cria as tabelas novas de estoque de peças/entrega e os enums correspondentes, nenhuma alteração em tabela existente).
...
- **Reconciliação de material do retrabalho compara contra a estimativa original do Job, não contra o que foi de fato consumido no ciclo anterior**: a nova `ProductionOrder` de retrabalho (Sprint 7) é vinculada ao MESMO `Job` da ordem original (para reaproveitar `completeProduction` sem recriar a lógica de reconciliação de estoque) — mas `completeProduction` reconcilia sempre contra `JobFilament.gramsUsed` (a estimativa original da calculadora), não contra o `gramsActual` já apontado no ciclo anterior. Na prática, isso significa que as gramas reais informadas na conclusão do retrabalho são comparadas com a estimativa original, o que pode super ou subestimar o consumo adicional real do retrabalho em relação ao que já havia sido consumido. Correção futura, se isso se mostrar um problema real: um `Job`/estimativa dedicado por ciclo de retrabalho, em vez de reaproveitar o mesmo `Job` da ordem original.
