...
Currently implemented: the Fundação only (auth, RBAC-by-action, audit trail, app shell). Everything else (customers, CRM Kanban, 3D pricing calculator, production, quality, inventory, finance) is scaffolded as empty module folders awaiting their sprint.
